...
//    var setTooltipsOnColumnHeader = function (grid, iColumn, text) {
  //      var thd = $("thead:first", grid[0].grid.hDiv)[0];
    ...
    $("#jqGrid").jqGrid({
        ...
        headertitles: true,
        hiddengrid: true,
        colNames: ['P', 'PeriodeZZ', 'Opdrachtgever', 'Uren', 'Factuur', '', 'Datum', 'Vervalt', 'Betaald', 'Factuur', 'Voorschot', 'Co&ouml;peratie', 'Dienstverlening', '', 'Resteert', 'Btw', 'Factuur ADZ', '', 'C', 'F'],
        colModel: [
            ...
            { label: 'FactuurNr', name: 'Factuurnr', align: 'right', width: 65 },
            {
                ...
                formatter: function (cellvalue, options, rowObject) {
					...
					//nieuwpdf
                    var jaar = rowObject[22];
                    jaar = (jaar == undefined ? rowObject.jaar : jaar);			
                    var werknemer_id = rowObject[23];
                    werknemer_id = (werknemer_id == undefined ? rowObject.werknemer_id : werknemer_id);
            ...
            { label: 'FactuurADZ', name: 'FactuurADZ', align: 'right', width: 50 },
            {
                ...
                formatter: function (cellvalue, options, rowObject) {
					...
					//nieuwpdf
                    var jaar2 = rowObject[22];
                    jaar2 = (jaar2 == undefined ? rowObject.jaar2 : jaar2);
                    var werknemer_id2 = rowObject[23];
                    werknemer_id2 = (werknemer_id2 == undefined ? rowObject.werknemer_id2 : werknemer_id2);
					
                    //return '<span class="div-icon-pdf2" title="klik om het bestand te downloaden" onclick="ajaxPdfAdz(' + veldwaarde_als_string + ');"></span>';
					return '<span class="div-icon-pdf2"  title="klik om het bestand te downloaden --2--" onclick="ajaxPdfAdz(' + veldwaarde_als_string  + ',' + '\'' + werknemer_id2  + '\',' +  '\'' + jaar2  + '\'' + ', 1);"></span>';
			...
			{  //dit wordt F-actoring
                label: 'F', name: 'HeeftFactoring', width: 20,
                formatter: function (cellvalue, options, rowObject) {
                    var factuurnummer = rowObject[4];
                    factuurnummer = (factuurnummer == undefined ? rowObject.Factuurnr : factuurnummer);
